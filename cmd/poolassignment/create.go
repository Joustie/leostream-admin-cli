...
var createCmd = &cobra.Command{
	Use:   "create",
	Short: "Create Leostream pool assignment",
	Long: `Create Leostream pool assignment by ID. For example:
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Println("Error creating PoolAssignment: ", err)
			return
		}
...
func init() {
	createCmd.Flags().StringArrayVarP(&ismemberof, "isMemberOf", "", []string{}, "SAML attribute or AD group to add to the pool assignment(can be used multiple times)")
	createCmd.Flags().String("pool_id", "", "Pool to associate with  the pool assignment")
	createCmd.Flags().String("protocolplan_id", "", "Protocol plan to associate with the pool assignment")
	createCmd.Flags().String("powerplan_id", "", "Power plan to associate with  the pool assignment")
	createCmd.Flags().String("releaseplan_id", "", "AD group to associate with the pool assignment")
	createCmd.Flags().String("display_mode", "9", "Way desktop and pool are displayed to the user")
	/*
	...
	*/
	createCmd.Flags().String("offer_filter", "0", "Filter how desktops are offered to the user")
	/*
	Offer quantity options:	
	The number of desktops to offer to a user at login
	*/
	createCmd.Flags().String("offer_quantity", "1", "Number of desktops are offered to the user")
	/*
	Start_if_stopped  options:
	0: Do not start stopped or suspended desktops
	1: Start stopped or suspended desktops
	*/
	createCmd.Flags().String("start_if_stopped", "0", "Start stopped or suspended desktops")
	...
	poolassignmentCmd.AddCommand(createCmd)
